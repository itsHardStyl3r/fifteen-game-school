...
progcmd='java -jar ./extras/puzzleval.jar'
...
for filename in "$games_dir"/*; do
    if [[ -f "$filename" && "$(basename "$filename")" =~ $sol_filename_regex ]]; then
        init_filename="$games_dir/${BASH_REMATCH[1]}.txt"
